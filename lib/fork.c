...
pgfault(struct UTrapframe *utf)
{
	...
	// inc/mmu.h has page fault error codes
	if (!(err & FEC_WR)) {
		panic("pgfault: faulting access to copy-on-write page");
	}

	// perms on VA of current page dir
...
duppage(envid_t envid, unsigned pn)
{
	...
	//if (!(uvpt[pn] & PTE_W)) {
	//	sys_page_map(0, addr, envid, addr, PTE_P | PTE_U);
	//	panic("duppage: faulting access to copy-on-write page");
	//}
